...
# MAGIC %sql
# MAGIC --cdm is common data model
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS delta.`/mnt/silver/patients` (
...
silver_patients_df = spark.read.format('delta').load('/mnt/silver/patients')
silver_patients_df.createOrReplaceTempView('silver_patients')

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Step 1: Mark existing records as historical (is_current = false) for patients that will be updated
# MAGIC MERGE INTO silver_patients AS target
...
# MAGIC %sql
# MAGIC select count(*),Patient_Key from delta.`/mnt/silver/patients`
