...
# MAGIC     datasource,
# MAGIC     CASE 
# MAGIC         WHEN ClaimID IS NULL OR TransactionID IS NULL OR PatientID IS NULL 
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS delta.`/mnt/silver/claims` (
...
silver_claims_df = spark.read.format("delta").load("/mnt/silver/claims")

# Register the Silver Layer data as a temporary SQL view for SQL operations
silver_claims_df.createOrReplaceTempView("silver_claims")
...
# MAGIC %sql
# MAGIC select * from delta.`/mnt/silver/claims`;
