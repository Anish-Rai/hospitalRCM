...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS delta.`/mnt/silver/encounters` (
...
silver_encounters_df = spark.read.format("delta").load("/mnt/silver/encounters")

# Register the Silver Layer data as a temporary SQL view for SQL operations
silver_encounters_df.createOrReplaceTempView("silver_encounters")
...
# MAGIC %sql
# MAGIC select SRC_EncounterID,datasource,count(patientid) from  delta.`/mnt/silver/encounters`
# MAGIC group by all
